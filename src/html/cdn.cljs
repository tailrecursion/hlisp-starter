(ns hlisp.demo.cdn
  (:use
    ...
    [flapjax.hlisp  :only [isE? isB? E->B B->E initE doE consE applyE caseE
                           ...
                           onMouseOutE onHoverE onActiveE domValueB domAttr
                           domRemoveAttr domCss domAddClass domRemoveClass
                           domToggleClass domToggle]]
    [flapjax.dom :only [id! value!]])
  (:use-macros
    [hlisp.macros :only [def-values]]))

(def response1 [["http://blah.com/broken-image.png"]
                ["http://blah.com/broken-image.png"
                 "http://tailrecursion.com/~alan/pix/froghat.jpg"]
                ["http://tailrecursion.com/~alan/pix/geysercar.jpg"
                 "http://tailrecursion.com/~alan/pix/geysercar.jpg"]
...
(def-values
  {search-input :element search-b :behavior}
  (make-input input))

(def-values
  {search-button :element search-e :estream}
  (make-clickable-element input))
